...
{
#ifdef __EBCANDROID__
    // EBC: Android USB host - open device via file descriptor passed from Java layer
    if (android_usb_fd != -1) {
        sdr_dev_t *dev = calloc(1, sizeof(sdr_dev_t));
        if (!dev) {
            WARN_CALLOC("sdr_open_rtl(android)");
            return -1;
        }
#ifdef THREADS
        pthread_mutex_init(&dev->lock, NULL);
        ...
        dev->sample_size  = sizeof(uint8_t) * 2; // CU8
        dev->sample_signed = 0;
        dev->dev_info = strdup("{\"vendor\":\"Android\",\"product\":\"USB Host\",\"serial\":\"n/a\"}");
        *out_dev = dev;
        return 0;
    }
#endif
