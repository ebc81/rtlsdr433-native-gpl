...
static void sdr_callback(unsigned char *iq_buf, uint32_t len, void *ctx)
{
    ...
    // Periodic diagnostic: log every 10th IQ buffer to confirm data flow to demodulator
    if (cfg->watchdog % 10 == 1) {
        print_logf(LOG_DEBUG, __func__,
                "IQ frame #%d len=%u n_samples=%lu",
                (int)cfg->watchdog, (unsigned)len, (unsigned long)n_samples);
    }
...
static void acquire_callback(sdr_event_t *ev, void *ctx)
{
    ...
    // On Android the mongoose socketpair (UDP loopback) is unreliable.
    // Call sdr_callback directly from the SDR acquire thread instead.
    r_cfg_t *cfg = (r_cfg_t *)ctx;
    if (ev->ev == SDR_EV_DATA && ev->buf && ev->len > 0) {
        sdr_callback((unsigned char *)ev->buf, ev->len, cfg);
    }
#else
...
#ifdef __EBCANDROID__
    r = sdr_start(cfg->dev, acquire_callback, (void *)cfg,
...
#ifdef __EBCANDROID__
    announce_device_stat(cfg->dev_state);
#endif
    return r;
}
...
static void timer_handler(struct mg_connection *nc, int ev, void *ev_data)
{
    ...
    case MG_EV_TIMER: {
        ...
        print_log(LOG_DEBUG, "timer_handler", "MG_EV_TIMER");
...
// =============================================================================
// EBC Android Entry Point
// Activated only when __EBCANDROID__ is defined (set in CMakeLists.txt).
// Provides lifecycle hooks called from android_bridge.c via rtlsdr433.cpp (JNI).
// =============================================================================
#ifdef __EBCANDROID__
...
char android_gain_str[64] = "";          // e.g. "" = auto, "10.5" = 10.5 dB
int android_frequency_hz  = DEFAULT_FREQUENCY;
int android_sample_rate   = DEFAULT_SAMPLE_RATE;
int android_ppm           = 0;
...
//
// NOTE: We intentionally do NOT use mg_add_sock_opt(INVALID_SOCKET, timer_handler) here.
// On Android the mongoose select()-based poll does not reliably fire MG_EV_TIMER for
// INVALID_SOCKET "timer-only" connections because select(0,...) may not return on every
// call across all Android kernel versions.  Instead we drive the watchdog with a plain
// mg_time() comparison inside the poll loop — same logic as timer_handler, no mongoose
// timer dependency.
static int android_run_sdr_loop(r_cfg_t *cfg)
{
    __android_log_print(ANDROID_LOG_INFO, "RTL433", "android_run_sdr_loop: calling start_sdr (fd=%d)", android_usb_fd);
    int r = start_sdr(cfg);
    if (r < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "RTL433", "android_run_sdr_loop: start_sdr FAILED r=%d", r);
        return r;
    }
    __android_log_write(ANDROID_LOG_INFO, "RTL433", "android_run_sdr_loop: start_sdr OK, entering event loop");

    time(&cfg->hop_start_time);

    // Watchdog timing — mirrors the timer_handler logic in the original code.
    // First tick after 2.5 s (STARTING → GRACE check), then every 1.5 s.
    double next_watchdog = mg_time() + 2.5;

    // Ensure the mongoose manager is allocated and initialised before polling.
    // cfg->mgr is NULL at this point because get_mgr() has not been called in
    // the Android path — mg_mgr_poll(NULL, …) causes a SIGSEGV at offset 0x24.
    get_mgr(cfg);

    __android_log_write(ANDROID_LOG_INFO, "RTL433", "android_run_sdr_loop: event loop start");
    while (!cfg->exit_async) {
        // 500 ms poll tick — keeps mongoose infrastructure alive for any other
        // connections that may be added in future (e.g. TCP output sinks).
        mg_mgr_poll(cfg->mgr, 500);

        // ---- Android watchdog (replaces MG_EV_TIMER from timer_handler) ----
        double now = mg_time();
        if (now >= next_watchdog) {
            next_watchdog = now + 1.5;
            __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "android_run_sdr_loop: watchdog tick");

            if (cfg->watchdog != 0) {
                // IQ data is flowing — advance state on first confirmed tick.
                if (cfg->dev_state == DEVICE_STATE_STARTING
                        || cfg->dev_state == DEVICE_STATE_GRACE) {
                    cfg->dev_state = DEVICE_STATE_STARTED;
                    time(&cfg->sdr_since);
                    //announce_device_stat(cfg->dev_state);
                    __android_log_write(ANDROID_LOG_INFO, "RTL433",
                            "android_run_sdr_loop: device STARTED (IQ data confirmed)");
                }
                cfg->watchdog = 0;
                announce_device_stat(cfg->dev_state);
            } else {
                // No IQ data received since last tick.
                if (cfg->dev_state == DEVICE_STATE_STARTING) {
                    // Allow one extra tick before giving up.
                    cfg->dev_state = DEVICE_STATE_GRACE;
                    //announce_device_stat(cfg->dev_state);
                    __android_log_write(ANDROID_LOG_WARN, "RTL433",
                            "android_run_sdr_loop: watchdog GRACE — no IQ data yet");
                } else if (cfg->dev_state == DEVICE_STATE_GRACE
                        || cfg->dev_state == DEVICE_STATE_STARTED) {
                    __android_log_print(ANDROID_LOG_ERROR, "RTL433",
                            "android_run_sdr_loop: watchdog TIMEOUT (state=%d) — stopping SDR",
                            cfg->dev_state);
                    if (cfg->dev_state != DEVICE_STATE_STOPPED) {
                        cfg->exit_async = 1;
                        cfg->exit_code  = 3;
    ...
    __android_log_write(ANDROID_LOG_INFO, "RTL433", "android_run_sdr_loop: event loop exited (exit_async set)");

    sdr_stop(cfg->dev);
    __android_log_write(ANDROID_LOG_INFO, "RTL433", "android_run_sdr_loop: sdr_stop done");
...
// Main native run function — blocking. Called directly from the JNI thread.
...
void rtl433_start(void)
{
    ...
    r_cfg_t *cfg = &g_cfg;
    r_init_cfg(cfg);
    __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "rtl433_start: r_init_cfg done");

    cfg->conversion_mode = (conversion_mode_t)android_conversion;
    cfg->frequency[0]    = (uint32_t)android_frequency_hz;
    cfg->frequencies     = 1;
    cfg->samp_rate       = (uint32_t)android_sample_rate;
    cfg->ppm_error       = android_ppm;
    //cfg->verbosity      = LOG_TRACE; // default log level for Android (can be overridden by config file)
    //cfg->demod->detect_verbosity = LOG_TRACE;
    // Fix: use strdup so r_free_cfg can safely free() this pointer.
    // Direct pointer to the static android_gain_str buffer would cause UB when free() is called.
    if (android_gain_str[0] != '\0')
        cfg->gain_str = strdup(android_gain_str);

    // Digital AGC: enable RTL2832U chip digital gain compensation if requested.
    // Passed via android_agc_mode (1 = enable). Applies via sdr_apply_settings() in start_sdr().
    if (android_agc_mode) {
        cfg->settings_str = strdup("digital_agc=1");
    ...
    cfg->demod->squelch_offset = 1;
    cfg->fsk_pulse_detect_mode = FSK_PULSE_DETECT_AUTO;


    __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "rtl433_start: registering device protocols");
    register_all_protocols(cfg, 0);
    __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "rtl433_start: protocols registered");

    // Enable FM demodulation if any registered protocol requires FSK.
    // main() does this after register_all_protocols; we must replicate it here.
    for (size_t _i = 0; _i < cfg->demod->r_devs.len; ++_i) {
        r_device *_r = cfg->demod->r_devs.elems[_i];
        if (_r && _r->modulation >= FSK_DEMOD_MIN_VAL) {
            cfg->demod->enable_FM_demod = 1;
            break;
        }
    }
    __android_log_print(ANDROID_LOG_INFO, "RTL433",
            "rtl433_start: enable_FM_demod=%d, r_devs.len=%zu",
            cfg->demod->enable_FM_demod, cfg->demod->r_devs.len);

    // Add Android output: JSON delivered via JNI callback (no stdout)
    android_bridge_add_output(cfg);
    __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "rtl433_start: android output bridge added");
    ...
    __android_log_write(ANDROID_LOG_DEBUG, "RTL433", "rtl433_start: start_outputs done");

    // Blocking call — returns when exit_async is set or on error
    int ret = android_run_sdr_loop(cfg);
