...
typedef struct r_cfg {
    ...
    int sr_execopen;
    volatile int watchdog; ///< SDR acquire stall watchdog — written from libusb pthread, read from event loop thread
    /* global stats */
...
// =============================================================================
// EBC Android API — declared here so android_bridge.c and rtlsdr433.cpp can use them.
// Implemented in rtl_433.c (#ifdef __EBCANDROID__ block at the end of the file).
// =============================================================================
...
// Config vars (set before calling rtl433_start)
extern int  android_usb_fd;
extern char android_gain_str[64];
extern int  android_frequency_hz;
...
// Callbacks — implemented in rtlsdr433.cpp (JNI layer)
void announce_output_line(const char *line);
